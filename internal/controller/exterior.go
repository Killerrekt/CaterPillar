...
func CreateExterior(c *fiber.Ctx) error {
	var req model.GetExterior
	...
	err = services.CreateExterior(ctx, req)
	if err != nil {
...
func AddExteriorImage(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(model.Response{Message: "No file passed under key exterior", Status: false})
	}

	tires := files.File["exterior"]
	var images string
	...
	ctx := context.Background()
	err = services.InsertExteriorImage(ctx, images, uint32(uint_id))
	if err != nil {
...
func AddExteriorImages(c *fiber.Ctx) error {
	id := c.Params("inspection_id", "")
	if id == "" {
		return c.Status(fiber.StatusBadRequest).JSON(model.Response{Message: "Bad request alert", Status: false})
	}
	...
	ctx := context.Background()
	err = services.InsertExteriorImages(ctx, images, uint32(uint_id))
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(model.Response{Message: "Failed to save the images id", Status: false})
	}
...
func GetExterior(c *fiber.Ctx) error {
	id := c.Params("inspection_id", "")
	if id == "" {
		return c.Status(fiber.StatusBadRequest).JSON(model.Response{Message: "Bad request alert", Status: false})
	}
	...
	ctx := context.Background()
	tire, err := services.GetExterior(ctx, uint32(uint_id))
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(model.Response{Message: "Something went wrong", Status: false})
	}
